...
def test_for_cmd(seq,cmds,callname,skip=False):
    ...
    for i in range(len(cmds['attributes'])):
        if cmds['attributes'][i][0]=='opt':#opt
            exit_loop=False
            index=None
            cnt=0
        ...
        elif cmds['attributes'][i][0]=='opt_cmd':#opt_cmd
            exit_loop=False
            index=None
            cnt=0
            ...
            return True,output
        elif cmds['attributes'][i][0]=='str':#str
        ...
        elif cmds['attributes'][i][0]=='valN':#valN
            num=seq
            exit_loop=False
            num_from_seq=''
        ...
        elif cmds['attributes'][i][0]=='valS':#valS
            num=seq
            exit_loop=False
            string_from_seq=''
            for n in range(len(num)):
        ...
        elif cmds['attributes'][i][0]=='valF':#valF
            num=seq
            exit_loop=False
            num_from_seq=''
            for n in range(len(num)):
    ...
    for x in range(len(commands)):
        if not end:
            ...
            if not stat:
                ...
                if commands[x][int((len(commands[x])-1))][0]=='function':
                    function(output,commands[x])
    if counter==len(commands):
        raise Exception('Inavlid command input')
      
if __name__=='__main__':
    valid_commands=[('/shop',('opt_cmd',{'sell':[(('opt',('potatos','pumkins','melons','carrots'),'invalid item'),('valF',0.10,10000.0,'invalid price'))],'buy':[(('opt',('potatos','pumkins','melons','carrots'),'invalid item'),('valF',0.10,10000.0,'invalid price'))]},'invalid selection'))]
    print(make_cmd(valid_commands[0][0],valid_commands[0])['structure'],'\n')
    while True:
        cmd=input('>>> v2.2 command maker 2017 make_cmds5.pyw>>>')
